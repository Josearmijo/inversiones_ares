...
function enviarCorreoConfirmacion($para, $nombre, $monto, $meses, $tipo_operacion) {
    ...
    try {
                ...
                <div style='padding: 30px; background: #f9f9f9;'>
                    ...
                    <div style='background: white; padding: 20px; border-radius: 10px; margin: 20px 0; box-shadow: 0 2px 5px rgba(0,0,0,0.1);'>
                        ...
                        <table style='width: 100%;'>
                            <tr>
                                <td style='padding: 10px 0; border-bottom: 1px solid #eee;'><strong>Tipo de Operacion:</strong></td>
                                <td style='padding: 10px 0; border-bottom: 1px solid #eee;'>" . ucfirst(str_replace('_', ' ', $tipo_operacion)) . "</td>
                            </tr>
                            <tr>
                                <td style='padding: 10px 0; border-bottom: 1px solid #eee;'><strong>Monto Pagado:</strong></td>
                                <td style='padding: 10px 0; border-bottom: 1px solid #eee; color: #28a745; font-weight: bold;'>$" . number_format($monto, 2) . " USD</td>
                            </tr>
                            <tr>
                                <td style='padding: 10px 0; border-bottom: 1px solid #eee;'><strong>Meses:</strong></td>
                                <td style='padding: 10px 0; border-bottom: 1px solid #eee;'>$meses_formateados</td>
                            </tr>
                            <tr>
                    ...
                    <div style='background: #fff3cd; border: 1px solid #ffc107; padding: 20px; border-radius: 10px; margin: 20px 0;'>
                        ...
                        <p style='margin-bottom: 0;'>Su pago se encuentra <strong>pendiente de verificacion</strong> por parte del administrador del condominio. Una vez verificado, su deuda sera actualizada.</p>
                    </div>
                    
                    <p style='color: #666; font-size: 14px;'>Si tiene alguna consulta, no dude en contactar al administrador de su condominio.</p>
                    
                    <hr style='border: none; border-top: 1px solid #ddd; margin: 30px 0;'>
                    
                    <p style='text-align: center; color: #999; font-size: 12px;'>
                        © " . date('Y') . " Inversiones Ares - Todos los derechos reservados
                    </p>
                </div>
            </div>
        </body>
        ...
        $mail->AltBody = "Transaccion Realizada\n\nEstimado/a $nombre,\n\nSu transaccion ha sido registrada exitosamente.\n\nDetalles del Pago:\n- Tipo de Operacion: " . ucfirst(str_replace('_', ' ', $tipo_operacion)) . "\n- Monto Pagado: $" . number_format($monto, 2) . " USD\n- Meses: $meses_formateados\n- Fecha: " . date('d/m/Y H:i:s') . "\n\nEstatus: Verificacion Pendiente\n\nSu pago se encuentra pendiente de verificacion por parte del administrador.";
...
if ($action === 'pagar') {
    ...
    if (empty($inmueble_id) || !is_numeric($inmueble_id) || intval($inmueble_id) <= 0) {
        echo json_encode(['success' => false, 'error' => 'ID de inmueble invalido: ' . $inmueble_id]);
        exit;
    }
    ...
    try {
        ...
        if ($tipo_operacion === 'completo') {
            ...
            $mes_pago_json = json_encode([['mes' => $mes_pago, 'anio' => intval($anio_pago), 'monto' => $monto_completo]]);
            ...
            $debug_info['pago_completo'] = ['id_insertado' => $id_insertado];
        ...
        elseif ($tipo_operacion === 'abono') {
            ...
            $mes_abono_json = json_encode([['mes' => $mes_pago, 'anio' => intval($anio_pago), 'monto' => $monto_abono]]);
            $meses_pagados[] = $mes_pago . ' ' . $anio_pago;
            ...
            $id_insertado = $condominioConn->lastInsertId();
            $pagos_insertados[] = $id_insertado;
            $debug_info['pago_abono'] = ['id_insertado' => $id_insertado];
        ...
        elseif ($tipo_operacion === 'ambos') {
            ...
            $mes_abono_json = json_encode([['mes' => $mes_abono, 'anio' => intval($anio_abono), 'monto' => $monto_abono]]);
            ...
            $stmtUpdate->execute([$id_pago_base, $id_abono, $id_pago_base]);
            
            $stmtUpdate2 = $condominioConn->prepare("UPDATE ingresos SET id_pago_base = ?, id_pago_relacionado = ? WHERE id_pago = ?");
            $stmtUpdate2->execute([$id_pago_base, $id_pago_base, $id_abono]);
            
            $debug_info['pago_completo'] = ['id_insertado' => $id_pago_base, 'monto' => $monto_completo];
            $debug_info['pago_abono'] = ['id_insertado' => $id_abono, 'monto' => $monto_abono];
        ...
        $resultado_correo = ['success' => false, 'error' => 'Email no verificado'];
