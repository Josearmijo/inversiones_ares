...
try {
    ...
    $user_id = $parts[0] ?? '';
    $db_name = $parts[1] ?? '';
    $volver_fn = $parts[2] ?? 'showGastosList';
    logDebug("user_id: $user_id, db_name: $db_name, volver_fn: $volver_fn");
...
<body>
    <div class="recibo-container">
        <button onclick="const fn = localStorage.getItem('volver_fn'); localStorage.removeItem('volver_fn'); if(window[fn]) { window[fn](); } else { window.location.href = 'index.html'; }" style="display:inline-flex;align-items:center;gap:8px;background:#28a745;color:white;padding:10px 20px;border-radius:8px;border:none;cursor:pointer;margin-bottom:15px;font-weight:600;font-size:14px;">
    ...
    <script>
        localStorage.setItem('volver_fn', '<?php echo $volver_fn; ?>');
    </script>
</body>
</html>