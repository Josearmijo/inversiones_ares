    ...
    try {
        ...
        container.innerHTML = `
            ...
            ${pagos.map(pago => {
                ...
                const estadoClass = estado === 'aprobado' ? 'success' : 'warning';
                const estadoIcon = estado === 'aprobado' ? 'check-circle' : 'clock';
                const estadoText = estado === 'aprobado' ? 'Aprobado' : 'Pendiente de Verificación';
                ...
                return `
                    <div class="card" style="margin-bottom: 15px; border-left: 4px solid ${estado === 'aprobado' ? '#28a745' : '#ffc107'};">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <div>
                                <span class="badge bg-${estadoClass}" style="font-size: 12px;">
                                    <i class="fas fa-${estadoIcon}"></i> ${estadoText}
                                </span>
                                <span class="badge bg-secondary" style="font-size: 11px; margin-left: 5px;">${tipoLabel}</span>
                            </div>
                            <div style="color: #666; font-size: 12px;">${fechaFormateada}</div>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 13px;">
                            ...
                            ${pago.total_bolivares > 0 ? `
                                <div><strong>Monto Bs.:</strong></div>
                                <div style="text-align: right;">${parseFloat(pago.total_bolivares).toFixed(2)} Bs.</div>
                            ` : ''}
                            
                            ${pago.referencia_op ? `
                                <div><strong>Referencia:</strong></div>
                                <div style="text-align: right;">${pago.referencia_op}</div>
                            ` : ''}
                        </div>
                        
                        ${estado === 'aprobado' && pago.id_pago ? `
...
function verRecibo(pagoId) {
    window.open(`../../../fpdf/recibo_ingresos.php?id=${pagoId}`, '_blank');
}
