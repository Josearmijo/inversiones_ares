    ...
    try {
        ...
        container.innerHTML = `
            <div class="card">
                ...
                <p class="text-muted">${currentInmueble?.nombre || ''} - ${gastos.length} registros</p>
            </div>
            
            ${gastos.length === 0 ? '<div class="alert alert-info">No hay gastos registrados</div>' : ''}
            ${gastos.map(g => `
                <div class="recibo-item" style="margin-bottom: 15px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; background: white;">
                    <div class="recibo-header" style="background: #28a745; color: white; padding: 10px; margin: -15px -15px 15px -15px; border-radius: 8px 8px 0 0;">
                        <span class="recibo-fecha" style="font-size: 16px; font-weight: bold;">${g.mes} ${g.anio}</span>
                    </div>
                    <div style="background: #f8f9fa; padding: 10px; margin-bottom: 10px; border-radius: 5px;">
                        <strong>Inmueble:</strong> ${g.inmueble_nombre || 'Inmueble ' + g.inmueble_id}
                    </div>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 5px; font-size: 13px;">
                        <div>Condominio:</div><div style="text-align:right"><strong>$${parseFloat(g.condominio || 0).toFixed(2)}</strong></div>
                        <div>Fondo Reserva:</div><div style="text-align:right"><strong>$${parseFloat(g.fondo_reserva || 0).toFixed(2)}</strong></div>
                        <div>Fecha Registro:</div><div style="text-align:right">${g.fecha_registro ? new Date(g.fecha_registro).toLocaleDateString('es-VE') : '-'}</div>
                    </div>
                    <div style="display: flex; justify-content: space-between; padding: 15px; background: #007bff; color: white; font-size: 20px; font-weight: bold; margin: 10px -15px -15px -15px; padding: 15px; border-radius: 0 0 8px 8px;">
                        <span>TOTAL:</span> <span>$${parseFloat(g.total_a_pagar_mes || 0).toFixed(2)}</span>
                    </div>
                </div>
            `).join('')}
        `;
...
function verRecibo(pagoId) {
    ...
    window.open(`${BASE_URL}../../fpdf/generar_recibo.php?id=${pagoId}&token=${token}`, '_blank');
}
