    ...
    try {
        ...
        container.innerHTML = `
            <div class="card">
                ...
                <button class="btn btn-info" onclick="showMisPagos()">
...
function showPagarForm() {
            ...
            <form id="pago-form">
                <div class="form-group">
                    ...
                    <select id="tipo_operacion" required>
                        ...
                        ${!tieneSoloUnMes ? '<option value="ambos">Pago Completo con Abono</option>' : ''}
                    </select>
                </div>
                
                <div class="form-group" id="seccion_mes_completo">
                    <label>Mes a Pagar (Completo) *</label>
                    <select id="mes_pago" required>
                        <option value="">Seleccionar...</option>
                        ${pendientes.map(d => `
                            <option value="${d.mes}-${d.anio}">${getMonthName(d.mes)} ${d.anio} - Pendiente: $${parseFloat(d.total_a_pagar_mes || 0).toFixed(2)}</option>
                        `).join('')}
                    </select>
                </div>
                
                <div class="form-group" id="seccion_mes_abono" style="display:none;">
                    <label>Mes para Abono *</label>
                    <select id="mes_abono">
                        <option value="">Seleccionar...</option>
                        ${pendientes.map(d => `
                            <option value="${d.mes}-${d.anio}">${getMonthName(d.mes)} ${d.anio} - Pendiente: $${parseFloat(d.total_a_pagar_mes || 0).toFixed(2)}</option>
                ...
                <div class="form-group" id="seccion_monto_abono" style="display:none;">
                    <label>Monto de Abono ($) *</label>
                    <input type="number" id="monto_abono" step="0.01" min="0.01" placeholder="0.00">
                </div>
                ...
                <div class="form-group" id="seccion_tasa_cambio" style="display:none;">
                    <label>Tasa de Cambio (Bs/$) *</label>
                    <input type="number" id="tasa_cambio" step="0.0001" placeholder="0.00">
                </div>
                
                <div class="form-group" id="seccion_monto_bs" style="display:none;">
                    <label>Monto Bolivares</label>
                    <input type="number" id="monto_bs" step="0.01" placeholder="0.00" readonly>
                </div>
    ...
    function calcularMontoBs() {
        ...
        if (tipoPago !== 'Transferencia' && tipoPago !== 'Pago Movil') return;
        
        const tipoOperacion = document.getElementById('tipo_operacion').value;
        const tasaCambio = parseFloat(document.getElementById('tasa_cambio').value) || 0;
        let montoDolares = 0;
        
        if (tipoOperacion === 'ambos') {
            montoDolares = parseFloat(document.getElementById('monto_abono').value) || 0;
        } else {
            montoDolares = parseFloat(document.getElementById('monto_pagar').value) || 0;
        }
        
        if (tasaCambio > 0 && montoDolares > 0) {
            const bs = Math.floor(montoDolares * tasaCambio * 100) / 100;
            document.getElementById('monto_bs').value = bs.toFixed(2);
        }
    }
    
    document.getElementById('monto_abono').addEventListener('input', function() {
        document.getElementById('monto_pagar').value = this.value;
        calcularMontoBs();
    });
    ...
    document.getElementById('monto_abono')?.addEventListener('input', calcularMontoBs);
    document.getElementById('tasa_cambio')?.addEventListener('input', calcularMontoBs);
    ...
    document.getElementById('mes_pago').addEventListener('change', function() {
        ...
        if (monto > 0 && (tipoOperacion === 'completo' || tipoOperacion === 'ambos')) {
            ...
            if (tipoOperacion === 'ambos') {
                document.getElementById('monto_abono').value = monto.toFixed(2);
            }
        }
    });
...
async function procesarPago() {
    ...
    try {
        ...
        if (tipoOperacion === 'abono' || tipoOperacion === 'ambos') {
...
async function showGastosList() {
    ...
    try {
        ...
        // Calculate totals
        const totalGeneral = gastos.reduce((sum, g) => sum + parseFloat(g.total_a_pagar_mes || 0), 0);
        
        container.innerHTML = `
            <div style="background: linear-gradient(135deg, #1e3a5f 0%, #2d5a87 100%); color: white; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                    <div>
                        <h2 style="margin: 0 0 5px 0; font-size: 20px;"><i class="fas fa-file-invoice-dollar"></i> Gastos por Inmueble</h2>
                        <p style="margin: 0; opacity: 0.8; font-size: 14px;">${currentInmueble?.nombre || 'Todos los Inmuebles'}</p>
                    </div>
                    <div style="text-align: right;">
                        <div style="font-size: 12px; opacity: 0.8;">Total Registros</div>
                        <div style="font-size: 28px; font-weight: bold;">${gastos.length}</div>
                    </div>
                </div>
            </div>
            
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px;">
                ${gastos.length === 0 ? '<div class="alert alert-info" style="grid-column: 1/-1;">No hay gastos registrados</div>' : ''}
                ${gastos.map(g => `
                    <div style="background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); overflow: hidden; transition: transform 0.2s, box-shadow 0.2s; cursor: pointer;" onmouseover="this.style.transform='translateY(-3px)';this.style.boxShadow='0 8px 25px rgba(0,0,0,0.12)';" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='0 4px 15px rgba(0,0,0,0.08)';">
                        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px 20px; display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <div style="font-size: 18px; font-weight: bold;">${g.mes} ${g.anio}</div>
                                <div style="font-size: 12px; opacity: 0.9;"><i class="fas fa-building"></i> ${g.inmueble_nombre || 'Inmueble ' + g.inmueble_id}</div>
                            </div>
                        ...
                        <div style="padding: 20px;">
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 15px;">
                                <div style="background: #f8f9fa; padding: 12px; border-radius: 8px; text-align: center;">
                                    <div style="font-size: 11px; color: #6c757d; text-transform: uppercase; margin-bottom: 4px;">Condominio</div>
                                    <div style="font-size: 16px; font-weight: bold; color: #1e3a5f;">$${parseFloat(g.condominio || 0).toFixed(2)}</div>
                                </div>
                                <div style="background: #f8f9fa; padding: 12px; border-radius: 8px; text-align: center;">
                                    <div style="font-size: 11px; color: #6c757d; text-transform: uppercase; margin-bottom: 4px;">Fondo Reserva</div>
                                    <div style="font-size: 16px; font-weight: bold; color: #28a745;">$${parseFloat(g.fondo_reserva || 0).toFixed(2)}</div>
                                </div>
                            </div>
                            <div style="background: #fff3cd; padding: 10px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                                <span style="color: #856404; font-weight: 600;"><i class="fas fa-calendar-alt"></i> ${g.fecha_registro ? new Date(g.fecha_registro).toLocaleDateString('es-VE') : 'Sin fecha'}</span>
                                <span style="background: #ffc107; color: #000; padding: 4px 10px; border-radius: 20px; font-size: 12px; font-weight: bold;">PENDIENTE</span>
                            </div>
                            <div style="background: linear-gradient(135deg, #1e3a5f 0%, #2d5a87 100%); color: white; padding: 15px; border-radius: 10px; display: flex; justify-content: space-between; align-items: center;">
                                <span style="font-size: 14px; font-weight: 600;">TOTAL A PAGAR</span>
                                <span style="font-size: 22px; font-weight: bold;">$${parseFloat(g.total_a_pagar_mes || 0).toFixed(2)}</span>
                            </div>
                            <button class="btn btn-primary" style="width: 100%; margin-top: 15px; padding: 12px; border-radius: 8px; border: none; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); font-weight: 600;" onclick="event.stopPropagation();verGasto(${g.id})">
                                <i class="fas fa-file-pdf"></i> Ver Recibo de Cobro
                            </button>
                        </div>
                    </div>
                `).join('')}
            ...
            ${gastos.length > 0 ? `
            <div style="background: white; border-radius: 12px; padding: 20px; margin-top: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.08);">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <div style="font-size: 14px; color: #6c757d;">TOTAL ACUMULADO</div>
                        <div style="font-size: 28px; font-weight: bold; color: #1e3a5f;">$${totalGeneral.toFixed(2)}</div>
                    </div>
                    <div style="background: #e7f5e7; padding: 15px; border-radius: 10px;">
                        <i class="fas fa-chart-line" style="font-size: 30px; color: #28a745;"></i>
                    </div>
                </div>
            </div>
            ` : ''}
...
// Ver gasto (recibo de cobro) - abre en nueva ventana con token de autenticación
function verGasto(gastoId) {
    ...
    console.log('currentCondominio:', currentCondominio);
...
async function showMisPagos() {
    ...
    try {
        ...
        console.log('Mis Pagos response:', text.substring(0, 1000));
        ...
        if (pagosAprobados.length === 0 && pagosPendientes.length === 0) {
            container.innerHTML = `
                <div class="card">
                    <h3><i class="fas fa-history"></i> Mis Pagos</h3>
        ...
        // Generar HTML para pagos pendientes
        let htmlPendientes = '';
        if (pagosPendientes.length > 0) {
            htmlPendientes = `
                <h4 style="color: #ffc107; margin-top: 20px;"><i class="fas fa-clock"></i> Pagos Pendientes de Verificación</h4>
                    ...
                    return `
                        <div class="card" style="margin-bottom: 15px; border-left: 4px solid #ffc107;">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                <span class="badge bg-warning" style="font-size: 12px; color: #000;">
                                    <i class="fas fa-clock"></i> Pendiente de Verificación
                                </span>
                                <span class="badge bg-secondary" style="font-size: 11px; margin-left: 5px;">${tipoLabel}</span>
                            </div>
                            <div style="font-size: 12px; color: #666;">${fechaFormateada}</div>
                            <div style="font-size: 14px; margin: 10px 0;">
                                <strong>Mes:</strong> ${pago.mes_pago_display || pago.mes_pago || 'N/A'}
                            </div>
                            <div style="font-size: 16px; font-weight: bold; color: #28a745;">$${montoMostrar}</div>
                        </div>
                    `;
                }).join('')}
            `;
        ...
        // Generar HTML para pagos aprobados
        let htmlAprobados = '';
        if (pagosAprobados.length > 0) {
            htmlAprobados = `
                <h4 style="color: #28a745; margin-top: 20px;"><i class="fas fa-check-circle"></i> Pagos Aprobados</h4>
                    ...
                    return `
                        <div class="card" style="margin-bottom: 15px; border-left: 4px solid #28a745;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 13px;">
                                ...
                                <div style="text-align: right; font-weight: bold; color: #28a745;">$${montoMostrar}</div>
                                <div><strong>Método:</strong></div>
                                <div style="text-align: right;">${pago.tipo_pago || 'N/A'}</div>
                            </div>
                            <button class="btn btn-primary btn-sm" style="margin-top: 15px;" onclick="verRecibo(${pago.id_pago})">
        ...
        container.innerHTML = `
            <div class="card">
                <h3><i class="fas fa-history"></i> Mis Pagos</h3>
                <p class="text-muted">Historial de pagos del inmueble: ${escapeHtml(currentInmueble?.nombre)}</p>
            </div>
            ${htmlPendientes}
            ${htmlAprobados}
            <button class="btn btn-secondary" onclick="showDashboard()">
...
// Ver recibo - abre en nueva ventana con token de autenticación
