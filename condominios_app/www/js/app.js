...
let currentInmueble = JSON.parse(localStorage.getItem('inmueble')) || null;
let currentDeudas = [];
...
function logout() { localStorage.removeItem('inmueble');
    user = null;
...
async function showDeudas(inmuebleId, nombreInmueble) {
    ...
    if (inmuebleId === undefined && currentInmueble && !isNaN(currentInmueble.id)) {
        inmuebleId = currentInmueble.id;
        nombreInmueble = currentInmueble.nombre;
    }

    if (inmuebleId === undefined || inmuebleId === null || isNaN(parseInt(inmuebleId))) {
        showHome();
        return;
    }

    currentInmueble = { id: parseInt(inmuebleId), nombre: nombreInmueble }; localStorage.setItem('inmueble', JSON.stringify(currentInmueble));
...
async function showPagarForm() {
    ...
    document.getElementById('tasa_cambio').addEventListener('input', function() {
        ...
        if (tasa > 0 && monto > 0) {
            document.getElementById('monto_bs').value = (monto * tasa).toFixed(2);
        }
    });
...
async function showGastosList(inmuebleIdParam) {
    showView('recibos-view');
    
    const container = document.getElementById('recibos-content');
    const inmuebleId = inmuebleIdParam || currentInmueble?.id || '';
    ...
    try {
        ...
        container.innerHTML = '<div style="background: linear-gradient(135deg, #1e3a5f 0%, #2d5a87 100%); color: white; padding: 20px; border-radius: 12px; margin-bottom: 20px;"><h2><i class="fas fa-file-invoice-dollar"></i> Gastos</h2><p>' + (currentInmueble?.nombre || 'Inmueble') + '</p></div><div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px;">';
        
        if (gastos.length === 0) {
            container.innerHTML += '<div class="alert alert-info">No hay gastos registrados</div>';
        } else {
            container.innerHTML += gastos.map(g => '<div style="background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); overflow: hidden; cursor: pointer;"><div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px;"><div style="font-size: 18px; font-weight: bold;">' + g.mes + ' ' + g.anio + '</div><div style="font-size: 12px; opacity: 0.9;"><i class="fas fa-building"></i> ' + (g.inmueble_nombre || 'Inmueble ' + g.inmueble_id) + '</div></div><div style="padding: 20px; text-align: center;"><div style="font-size: 14px; color: #666; margin-bottom: 5px;">TOTAL A PAGAR</div><div style="font-size: 28px; font-weight: bold; color: #28a745;">$' + parseFloat(g.total_a_pagar_mes || 0).toFixed(2) + '</div></div><div style="padding: 0 15px 15px 15px;"><button class="btn btn-primary" style="width: 100%; padding: 12px; border-radius: 8px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);" onclick="event.stopPropagation();verGasto(' + g.id + ')"><i class="fas fa-file-pdf"></i> Ver Recibo</button></div></div>').join('');
        }
...
async function showMisPagos(inmuebleIdParam) {
    showView('pagos-view');
...
async function procesarPago() {
    ...
    const successDiv = document.getElementById('pago-success');
    errorDiv.classList.add('hidden');
    successDiv.classList.add('hidden');
    
    const tipo_operacion = document.getElementById('tipo_operacion').value;
    const mes_pago = document.getElementById('mes_pago').value;
    const mes_abono = document.getElementById('mes_abono').value;
    const monto_pagar = document.getElementById('monto_pagar').value;
    const monto_abono = document.getElementById('monto_abono').value;
    const tipo_pago = document.getElementById('tipo_pago').value;
    const tasa_cambio = document.getElementById('tasa_cambio').value;
    const referencia = document.getElementById('referencia').value;
    const banco = document.getElementById('banco').value;
    const comprobante = document.getElementById('comprobante').files[0];
    
    if (!currentInmueble || !currentInmueble.id || isNaN(currentInmueble.id)) {
        errorDiv.textContent = 'Error: Debe seleccionar un inmueble primero';
        errorDiv.classList.remove('hidden');
        return;
    }
    
    if (!tipo_operacion || !tipo_pago || !referencia) {
    ...
    formData.append('deuda_id', '');
    formData.append('tipo_pago', tipo_pago);
    formData.append('tasa_cambio', tasa_cambio || 0);
    formData.append('referencia', referencia);
    formData.append('banco', banco);
    formData.append('tipo_operacion', tipo_operacion);
    
    if (tipo_operacion === 'completo') {
        const [mes, anio] = mes_pago.split('-');
        formData.append('monto', monto_pagar);
        formData.append('monto_completo', monto_pagar);
        formData.append('mes_pago', mes);
        formData.append('anio_pago', anio);
    } else if (tipo_operacion === 'abono') {
        const [mes, anio] = mes_abono.split('-');
        formData.append('monto_abono', monto_abono);
        formData.append('mes_pago', mes);
        formData.append('anio_pago', anio);
    } else if (tipo_operacion === 'ambos') {
        const [mesC, anioC] = mes_pago.split('-');
        const [mesA, anioA] = mes_abono.split('-');
        formData.append('monto_completo', monto_pagar);
        formData.append('monto_abono', monto_abono);
        formData.append('mes_pago', mesC);
        formData.append('anio_pago', anioC);
        formData.append('mes_abono', mesA);
    ...
    try {
        ...
        if (data.success) {
            successDiv.innerHTML = '<i class="fas fa-check-circle"></i> ' + data.mensaje;
            successDiv.classList.remove('hidden');
            errorDiv.classList.add('hidden');
            document.getElementById('pago-form').reset();
            if (data.mostrar_sweetalert) {
                Swal.fire({
                    title: data.sweetalert_title,
                    text: data.sweetalert_text,
                    icon: data.sweetalert_icon,
